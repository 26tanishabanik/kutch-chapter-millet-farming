...
def get_pH(start_date, end_date, roi):
  sentinel = ee.ImageCollection("COPERNICUS/S2_SR").filterBounds(roi).filterDate(start_date,end_date).filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',20)).mean()
...
def get_soil_salinity(start_date, end_date, roi):
  ...
  file_path = './images/Soil Salinity.tif'
...
def fetch_satellite_data(start_date, end_date, aoi_roi):
  ...
  windspeed_min, windspeed_max = get_windspeed(start_dt, end_dt, roi)
  canopy_cover_mean = get_canopy_cover(start_dt, end_dt, roi)
  soil_moisture_min, soil_moisture_max = get_soil_moisture(start_dt, end_dt, roi)
  humidity_min, humidity_max = get_humidity(start_dt, end_dt, roi)
  elevation_min, elevation_max = get_elevation(roi)
  soil_salinity_min_value, soil_salinity_max_value, soil_salinity_average_value = get_soil_salinity(start_dt, end_dt, aoi_roi[0])
  return temperature_min, temperature_max, pH_min, pH_max, rainfall_min, rainfall_max, windspeed_min, windspeed_max, soil_moisture_min, soil_moisture_max, humidity_min, humidity_max, elevation_min, elevation_max, canopy_cover_mean, soil_salinity_min_value, soil_salinity_max_value, soil_salinity_average_value
